...
export const fetchMarketReport = async () => {
  const [cities, news] = await Promise.all([
    getAllFuelPrices(),
    getLatestNews(),
  ]);
  ...
  const petrolPrices = cities.map((city) => city.fuels.petrol.regular);
  const dieselPrices = cities.map((city) => city.fuels.diesel.regular);

  const averagePetrol =
    petrolPrices.reduce((a, b) => a + b, 0) / petrolPrices.length;
...
export const fetchStateAnalytics = async () => {
  ...
  const analytics = Object.entries(states).map(([state, cities]) => {
    if (cities.length === 0) {
      throw new AppError(
        MESSAGES.ERROR.CITIES_NOT_FOUND,
        HTTP_STATUS_CODES.NOT_FOUND,
  ...
  return analytics.sort((a, b) => a.state.localeCompare(b.state));
};
